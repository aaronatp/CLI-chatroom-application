# this solution doesn't stop users from having duplicate usernames
# in fact, when it associates SharedChat instances with users, it assumes there are no duplicate usernames
# a more real-world solution would be to identify users (internally) by some hash
# (maybe of their username and the time/date of their account creation, for example)
# that would better ensure that users are uniquely identified internally

# the way that chat data is stored is not necessarily data safe
# users could make data additions at (almost) the same time and one
# users' data contribution might override another users'

# the user's username and password are saved to the dict of known users
# but they don't persist after the program stops running
# if they were going to persist, the username and dict could fairly easily
# be contained in another file or a database

# the benefit of using three classes (ChatApp, SharedChat, and SpeechBubble) - ChatApp
# handles the application administrative tasks (like logging users in and sending them to chatrooms),
# SharedChat manages how the chatrooms themselves work, and SpeechBubble organizes and beautifies
# each message

# asynch is used in SharedChat.join_chat. If there were multiple users in a
# chatroom, and one users sent a message, 'join_chat' would send the updated content
# to the parent class. The general purpose of using asynch - for the other users,
# asynch checks at one-second intervals to see if the parent's chatroom data
# is different from the local copies in their SharedChat (chatroom) instances.
# If some users' local copies of the chatroom data is different from the parent's copy,
# the local copies sync to the parent copy (i.e., they get any messages sent by other users).
# Since this happens at one-second intervals, this keeps the chats pretty much sync'd

# However, this program doesn't handle what happens if two people write to a chat 
# before either chat is merged (merge conflicts). It seems like there are probably
# best practices for this. I would be very happy to look into this - do you want me
# to extend this program to handle merge conflicts too?

# can I also please implement code tests and perhaps a database feature? 

# very bare-bones implementation of this, requiring only one built-in library
